...
public class DynamicShardingManager {
    ...
    private Set<String> fetchAllPictureTableNames() {
        ...
        wrapper.eq(Space::getSpaceType, SpaceTypeEnum.TEAM.getValue());

        Set<Long> spaceIds = spaceMapper.selectList(wrapper).stream().map(Space::getId).collect(Collectors.toSet());
        Set<String> tableNames =
            spaceIds.stream().map(spaceId -> LOGIC_TABLE_NAME + "_" + spaceId).collect(Collectors.toSet());
    ...
    private void updateShardingTableNodes() {
        ...
        String newActualDataNodes = tableNames.stream().map(tableName -> "pic_hub" + "." + tableName) // 确保前缀合法
            .collect(Collectors.joining(","));
        log.info("动态分表 actual-data-nodes 配置: {}", newActualDataNodes);

        ContextManager contextManager = getContextManager();
        ShardingSphereRuleMetaData ruleMetaData =
            contextManager.getMetaDataContexts().getMetaData().getDatabases().get(DATABASE_NAME).getRuleMetaData();
